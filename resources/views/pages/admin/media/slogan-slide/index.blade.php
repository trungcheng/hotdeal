...
@section('content')
    <div ng-controller="MediaController" ng-init="loadSloganSlideInit()">
        ...
        <section class="content-header" style="padding-top:30px;">
            <h1>
                ...
                <a href="{{ route('media-slogan-slide-create') }}" class="pull-right btn btn-success btn-sm">Thêm mới</a>
            </h1>
        </section>
                            ...
                            <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                                <div class="row" style="margin-bottom:10px;">
                                    ...
                                    <div class="col-sm-6">
                                        <div id="example1_filter" class="dataTables_filter" style="float:right;">
                                            <label>Search:
                                                <input my-enter="searchSloganSlideName()" ng-model="searchText" type="search" class="form-control input-sm" placeholder="Tìm kiếm...">
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-12" style="overflow-x:auto;">
                                        <table id="example1" class="table table-bordered table-hover table-striped dataTable" role="grid" aria-describedby="example1_info">
                                            <thead>
                                                <tr role="row">
                                                    ...
                                                    <th style="text-align:center !important;width:30%">Nội dung</th>
                                                    <th>Ngày tạo</th>
                                                    <th>Chức năng</th>
                                                </tr>
                                            </thead>
                                            <tbody ng-cloak>
                                                <tr role="row" class="@{{ ($odd) ? 'odd' : 'even' }}" ng-repeat="article in items track by $index">
                                                    <td class="sorting_1">@{{ $index + 1 }}</td>
                                                    <td style="text-align:center !important">
                                                        <img ng-src="@{{ article.image }}" style="width:70px;height:60px;">
                                                    </td>
                                                    <td>@{{ article.title }}</td>
                                                    <td class="intro" style="width:30%" ng-bind-html="article.intro"></td>
                                                    <td>@{{ article.created_at }}</td>
                                                    <td>
                                                        <a href="/admin/access/media/slogan-slides/edit/@{{ article.id }}" style="margin-right:5px;" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
                                                        <a ng-click="deleteSloganSlide(article, $index)" style="margin-left:5px;" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        ...
                                        <div ng-cloak ng-if="!loading && items.length === 0">
                                            <h5 style="font-size:16px;color:#f00;">Oops! Không tìm thấy slide!</h5>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row" ng-cloak ng-if="!loading && items.length > 0">
                                    <div class="col-sm-5" style="padding-top:5px;">
                                        <div class="dataTables_info" id="example1_info" role="status" aria-live="polite">Hiển thị từ <strong>@{{ from }}</strong> đến <strong>@{{ to }}</strong> của <strong>@{{ total }}</strong> slide</div>
                                    </div>
                                    <div class="col-sm-7">
